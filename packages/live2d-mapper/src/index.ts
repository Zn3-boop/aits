...
const EMOTION_MAP: Record<string, Partial<Live2DExpressionParams>> = {
...
export function emotionToLive2DParams(emotion: string): Live2DExpressionParams {
...
export function mapFaceMeshToLive2D(faceMesh: FaceMeshResult): Live2DExpressionParams {
...
export function applyToLive2DModel(model: any, params: Live2DExpressionParams): void {
  if (!model?.internalModel?.coreModel) return;
  const core = model.internalModel.coreModel;
  const set = (id: string, v: number) => {
    try { core.setParameterValueById(id, v); } catch { /* ignore missing param */ }
  };
